...
WHERE 
	...
	--LAPTOP-BFUTVVNG\SQLEXPRESS
	--Запрос для связи Suspect - InvolvedIn - Crime:
	SELECT 
    S.id AS SuspectId,
    S.name AS SuspectName,
    C.id AS CrimeId,
    C.type AS CrimeType,
    II.role AS InvolvementRole
FROM 
    Suspect AS S,
    InvolvedIn AS II,
    Crime AS C
...
--Запрос для связи Officer - Investigates - Crime:
SELECT 
    O.id AS OfficerId,
...
--Запрос для связи Crime - TookPlaceAt - Location:
SELECT 
    C.id AS CrimeId,
    C.type AS CrimeType,
    L.id AS LocationId,
...
--Запрос для связи Suspect - SeenAt - Location:
SELECT 
    S.id AS SuspectId,
    S.name AS SuspectName,
    L.id AS LocationId,
    L.name AS LocationName,
    SA.seen_date AS SeenDate,
    SA.method AS DetectionMethod
FROM 
    Suspect AS S,
    SeenAt AS SA,
    Location AS L
...
--Запрос для связи Evidence - EvidenceInCrime - Crime:
SELECT 
    E.id AS EvidenceId,
...
--Запрос для связи Suspect - LinkedTo - Evidence:
SELECT 
    S.id AS SuspectId,
    S.name AS SuspectName,
    E.id AS EvidenceId,
    E.type AS EvidenceType,
    LT.relation_type AS RelationType
FROM 
    Suspect AS S,
    LinkedTo AS LT,
    Evidence AS E
...
--Запрос для связи Officer - WorkedTogether - Officer:
SELECT 
    O1.id AS Officer1Id,
    O1.name AS Officer1Name,
    O2.id AS Officer2Id,
    O2.name AS Officer2Name,
    WT.collaboration_date AS CollaborationDate,
    WT.cases_count AS CasesCount
FROM 
    Officer AS O1,
    WorkedTogether AS WT,
    Officer AS O2
WHERE 
    MATCH(O1-(WT)->O2)
ORDER BY 
    WT.collaboration_date DESC;